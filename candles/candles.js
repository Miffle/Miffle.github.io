...
let candlePrices = [500, 500, 500, 500, 500, 500];
let items = [0, 0, 0, 0, 0, 0];
let costItems = [0, 0, 0, 0, 0, 0];
let totalCost = 0;

let buttonIds = ["btn1", "btn2", "btn3", "btn4", "btn5", "btn6"];
let minButtonIds = ["min_btn1", "min_btn2", "min_btn3", "min_btn4", "min_btn5", "min_btn6"];
let labelIds = ["count1", "count2", "count3", "count4", "count5", "count6"];
...
for (let i = 0; i < buttonIds.length; i++) {
    ...
    button.addEventListener("click", function () {
        items[i]++;
        costItems[i] = candlePrices[i] * items[i];
        SetText(label, items[i], costItems[i]);
        MainButtonShow(candlePrices[i]);
    });
